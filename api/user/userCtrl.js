...
function Request() {
    var _getRequest = (endpoint, options) => {
        return new Promise( ( resolve, reject ) => {
            ...
            req.onerror = () => {
                reject( {result:-1, message:'Network error'} );
            };
    ...
    var _postRequest = (endpoint, options) => {
        return new Promise( ( resolve, reject ) => {
            var req = new XMLHttpRequest();
    ...
    this.get = async ( endpoint, options ) => {
        return await _getRequest( endpoint, options );
    }
    
    this.post = async ( endpoint, options ) => {
        return await _postRequest( endpoint, options );
    }
};
...
var loginCtrl = {
    ...
    swipe: null,
    previewAvatar: (avatarFile, canvasName) => {
    ...
    previewImg: (avatarFile, canvasName) => {
        var url = null;
...
//            leftMenuBtn.removeClass('active');
    ...
    displaySkills: ( obj, prefix, level ) => {
        console.log( obj );
...
//                }
    ...
    listSkills: ( obj ) => {
        ...
        for( var key in obj ) {
            txt+=(obj[key].value?(obj[key].name+' '):'')
            if( obj[key].specs != null && typeof( obj[key].specs ) == 'object' ) {
    ...
    restorePassword: () => {
        ...
        req.get( urlService.password.restore, {email:email} )
            .then( (response) => {
                if( response.result ) {
                    alert( 'Ошибка восстановления пароля: '+response.description );
                } else {
            ...
            .catch( (response) => {
                alert( 'Ошибка восстановления пароля: '+response );
                console.log( 'Err:' );
    ...
    newPassword: () => {
        ...
        req.get( urlService.api + urlService.user.new_passw, {token:id, password: passw1} )
            .then( (response) => {
                if( !response.status ) {
            ...
            .catch( (response) => {
                alert( 'Ошибка смены пароля: '+response.status );
                console.log( 'Err:' );
                console.log( response );
            } );
    ...
    registration: function (form_name) {
        ...
        if( !loginCtrl.validateRegistrationFields( form ) ) return;
                ...
                $('#dayList_' + date).click(function () {
                    ...
                    for (var i in loginCtrl.events) {
                        ...
                        if (this.innerText == date)
                            ...
                            setTimeout( () => { loginCtrl.fillEventComments( obj.id );}, 1200 );
                        }
